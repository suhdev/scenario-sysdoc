...
export function Reducer(state, action) {
    ...
    if (v === START) {
        switch (action.type) {
            ...
            case TYPES.FINISH:
                newState = newState.withMutations((m) => {
                    let staff = m.get('staff'), isBrand = m.get('level') > 0, isStrategic = m.get('strategic') > 0, value = m.get('value'), complexity = m.get('complexity');
                    let technical = null, brand = null, roles = [];
